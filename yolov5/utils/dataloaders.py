...
# class LoadStreams:
#     def __init__(self, sources="file.streams", img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
#         """Initializes a stream loader for processing video streams with YOLOv5."""
#         torch.backends.cudnn.benchmark = True  # faster for fixed-size inference
#         self.mode = "stream"
#         self.img_size = img_size
#         self.stride = stride
...
#             fps = self.cap.get(cv2.CAP_PROP_FPS) % 100 or 30
#             self.fps[i] = fps
#             self.frames[i] = max(int(self.cap.get(cv2.CAP_PROP_FRAME_COUNT)), 0) or float("inf")
#             LOGGER.info(f"{st} Success ({self.frames[i]} frames {w}x{h} at {self.fps[i]:.2f} FPS)")

#             self.sources[i] = s  # Initialize sources attribute

#         self.transforms = transforms

#     def __iter__(self):
#         """Resets and returns the iterator for iterating over video frames."""
#         self.count = -1
#         return self

#     def __next__(self):
#         """Iterates over video frames, raising `StopIteration` when done."""
...
#         im0 = np.array(frame)  # Original frame
#         im = letterbox(frame, self.img_size, stride=self.stride, auto=True)[0]  # Resized frame

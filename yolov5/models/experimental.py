...
def replace_posix_path(storage, location):
    """Recursively replaces all pathlib.Path objects with str objects."""
    if isinstance(location, pathlib.PosixPath):
        return storage, pathlib.WindowsPath(str(location))
    
    if isinstance(location, str):
        return storage, location
    
    if isinstance(location, dict):
        return {k: replace_posix_path(v) for k, v in location.items()}
    if isinstance(location, (list, tuple)):
        return [replace_posix_path(x) for x in location]
    return storage, location
...
def attempt_load(weights, device=None, inplace=True, fuse=True):
    ...
    for w in weights if isinstance(weights, list) else [weights]:
        ...
        #     print("Model weights not found locally, attempting to download from Torch Hub...")
        #     print("Exception: ", exc)
            
        #        # Attempt to download the model weights from the Torch Hub
        # Get weights
        location = ""
        try:
            location = attempt_download(w)
            ...
            ckpt = torch.load(location, map_location="cpu")  # load
        except Exception as exc:
